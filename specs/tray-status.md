...
| If a turn contains a failed tool execution, then TrayGoblin shall report Attention needed with a content-free reason until the next turn starts; an unreadable selected state file shall also report Attention needed. | `cargo test --test monitor failed_tool_sets_attention_needed && cargo test --test monitor attention_needed_persists_until_next_turn_start && cargo test --test monitor missing_events_file_sets_state_unavailable` |
